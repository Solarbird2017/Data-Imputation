...
x = Variable(X_train)  # 将它们装载到Variable的容器里
y = Variable(y_train)  # 将它们装载到Variable的容器里
...
net = Net(n_feature=col_dim, n_hidden=10, n_output=col_dim)  # two classification has two n_features #实例化一个网络结构
print(net)
